...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if args.bridge_json is not None:
        ...
        # A scan case may place the physical N=1 crossing outside the first
        # Lagrangian shell.  The bridged phase-space profile and crossing remain
        # usable for an FP flux calculation, although the present one-shell
        # fluid coupling cannot implement that handoff.  Keep this distinction
        # explicit instead of discarding the physical FP case or relabelling the
        # bridge as a successful subgrid coupling.
        bridge_status = bridge.get("status")
        if bridge_status not in ("OK", "SUBGRID_MASS_FRACTION_TOO_LARGE"):
            raise RuntimeError(
                "hydrostatic bridge has no usable inner FP crossing: "
                f"{bridge_status}"
            )
    ...
    table_meta = {
        ...
        "fluid_interface_implementation_status": (
            "SUBGRID_COUPLING_READY"
            if bridge.get("status") == "OK"
            else "RESOLVED_FP_REPLACEMENT_REQUIRED"
        ),
