...
subroutine get_tnr_timescale_at_rh(tnr)
	...
	implicit none
	 ...
	 if (use_sidm) then
             ! SIDM relaxation clock (restored): replace gravitational Coulomb log
             ! with SIDM coupling kappa0*<lnLambda_eff> (mirrors sidm_params t_relax)
             sidm_coupling = sidm_kappa0() * log_lambda_eff_avg()
             tnr = 0.34*ctl%v0**3 / nm2_tot / sidm_coupling / 2d0/pi/1d6
	 else
	     lglambda=log(mbh)
