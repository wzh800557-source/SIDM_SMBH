...
def main():
    ...
    if bridge:
        if (bridge.get('status') not in ('OK','SUBGRID_MASS_FRACTION_TOO_LARGE')
                or bridge.get('schema')!='absolute-closure-hydrostatic-bridge-v3'):
            raise RuntimeError('hydrostatic bridge has no usable v3 FP crossing')
